...
import sharp from 'C:/AIProjects2026/liftingdairycourse/node_modules/sharp/lib/index.js';
import { writeFileSync } from 'fs';

// --- Data: past 12 months from today (Apr 18, 2026) ---
// Months from May 2025 to Apr 2026
const data = [
...
data.forEach((d, i) => {
  ...
  // Bar
  bars += `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${barWidth}" height="${barH.toFixed(1)}" fill="#4f86c6" rx="3"/>`;
  ...
  // X-axis label
  bars += `<text x="${(x + barWidth / 2).toFixed(1)}" y="${(paddingTop + chartH + 20).toFixed(1)}" text-anchor="middle" font-size="11" fill="#555" transform="rotate(45 ${(x + barWidth / 2).toFixed(1)} ${(paddingTop + chartH + 20).toFixed(1)})">${d.label}</text>`;
});
...
for (let t = 0; t <= tickCount; t++) {
  ...
  const y = toY(val);
  gridLines += `<line x1="${paddingLeft}" y1="${y.toFixed(1)}" x2="${W - paddingRight}" y2="${y.toFixed(1)}" stroke="#ddd" stroke-dasharray="4,4"/>`;
  gridLines += `<text x="${paddingLeft - 8}" y="${(y + 4).toFixed(1)}" text-anchor="end" font-size="11" fill="#666">${Math.round(val)}</text>`;
}

const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" style="background:#fff;font-family:Arial,sans-serif;">
  <!-- Title -->
  <text x="${W / 2}" y="40" text-anchor="middle" font-size="18" font-weight="bold" fill="#222">Workouts per Month (Past 12 Months)</text>

  <!-- Grid lines -->
  ${gridLines}

  <!-- Axes -->
  <line x1="${paddingLeft}" y1="${paddingTop}" x2="${paddingLeft}" y2="${paddingTop + chartH}" stroke="#999" stroke-width="1.5"/>
  <line x1="${paddingLeft}" y1="${paddingTop + chartH}" x2="${W - paddingRight}" y2="${paddingTop + chartH}" stroke="#999" stroke-width="1.5"/>

  <!-- Y-axis label -->
  <text x="20" y="${(paddingTop + chartH / 2).toFixed(1)}" text-anchor="middle" font-size="13" fill="#444" transform="rotate(-90 20 ${(paddingTop + chartH / 2).toFixed(1)})">Number of Workouts</text>

  <!-- X-axis label -->
  <text x="${W / 2}" y="${H - 5}" text-anchor="middle" font-size="13" fill="#444">Month</text>
